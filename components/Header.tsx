...
const Header: React.FC = () => {
  ...
  return (
    <header className={`fixed top-0 w-full z-50 flex justify-between items-center px-4 sm:px-6 md:px-10 lg:px-16 py-4 sm:py-5 transition-all duration-300 ease-in-out ${isScrolled ? 'scrolled' : ''}`} style={{ background: isScrolled ? 'rgba(255, 255, 255, 0.09)' : 'transparent', backdropFilter: 'blur(7px)', WebkitBackdropFilter: 'blur(7px)', borderBottom: '1px solid rgba(255, 255, 255, 0.44)' }}>
      ...
      <div className={`fixed inset-0 top-[84px] z-40 flex flex-col items-center space-y-6 py-8 md:hidden transition-opacity duration-300 ease-in-out ${isMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} style={{background: 'rgba(255, 255, 255, 0.2)', backdropFilter: 'blur(7px)', WebkitBackdropFilter: 'blur(7px)'}}>
        {navLinks.map(link => (
          <a key={link.href} href={link.href} onClick={handleLinkClick} className='text-xl font-medium hover:text-[#5A99FC] transition-colors min-h-[44px] flex items-center focus:outline-none focus:ring-2 focus:ring-[#5A99FC] focus:ring-offset-2 rounded px-2'>{link.label}</a>
        ))}
        <a href='https://calendly.com/pixel-pop-digital' target="_blank" rel="noopener noreferrer" onClick={handleLinkClick} className='btn-primary min-h-[44px] flex items-center justify-center text-lg'>Book Call</a>
      </div>
    </header>
  );
};
